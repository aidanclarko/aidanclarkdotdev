  ...
  return (
    <>
      <div className="flex justify-center items-center mt-20 z-40">
        <div className="w-[75vw] md:h-[30vw] rounded-xl shadow-2xl shadow-foreground bg-foreground flex flex-col">
                    ...
                    <div className="flex flex-col gap-3">
                      {listOfCommands.map((cmd) => (
                        <div key={cmd.cmd} className="flex flex-col">
                          <span className="text-green-400 font-mono">{cmd.cmd}</span>
                          <span className="text-sm text-gray-200">{cmd.desc}</span>
                        </div>
                      ))}
                    </div>
                    <Button slot="close" className="mt-10 text-background font-bold">
                      Close
                    </Button>
                  </Dialog>
                </Modal>
